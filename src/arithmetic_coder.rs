...
impl<W: Write, const N: usize> ArithmeticEncoder<W, N> {
    ...
    pub fn new(writer: W, state_table: &'static [State], num_contexts: usize, is_jb2: bool) -> Result<Self, ArithmeticError> {
        if num_contexts != N {
    ...
    pub fn flush(&mut self, with_marker: bool) -> Result<(), ArithmeticError> {
        ...
        if self.is_jb2 {
            ...
            self.byte_out(true)?;
            if with_marker {
                self.writer.as_mut().ok_or(ArithmeticError::WriterUnavailable)?.write_all(&[0xFF, 0xAC])?;
            }
        } else {
            for _ in 0..18 {
                self.byte_out(false)?;
        ...
        self.writer.as_mut().ok_or(ArithmeticError::WriterUnavailable)?.flush()?;
        self.finished = true;
        Ok(())
    }
