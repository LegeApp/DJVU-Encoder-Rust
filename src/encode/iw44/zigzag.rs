// src/encode/iw44/zigzag.rs

//! Zigzag location constants for IW44 encoding
//! 
//! These constants define the zigzag order for scanning coefficients in a 1024 liftblock.
//! The original C++ comment explains the generation algorithm:
//! 
//! ```c
//! int x=0, y=0;
//! for (int i=0; i<5; i++) {
...
/// Zigzag location lookup table for a 1024 liftblock
/// 
/// This array maps linear indices to zigzag scan order positions,
/// essential for coefficient ordering in wavelet transform encoding.
pub const ZIGZAG_LOC: [u16; 1024] = [
...
/// Get the zigzag location for a given index
/// 
/// # Arguments
/// * `index` - The linear index (0-1023)
/// 
/// # Returns
/// The zigzag scan order position for the given index
/// 
/// # Panics
/// Panics if index >= 1024
#[inline]
pub fn get_zigzag_loc(index: usize) -> u16 {
    ZIGZAG_LOC[index]
}

/// Get the zigzag location for a given index, with bounds checking
/// 
/// # Arguments
/// * `index` - The linear index
/// 
/// # Returns
/// `Some(position)` if index is valid, `None` otherwise
#[inline]
pub fn get_zigzag_loc_checked(index: usize) -> Option<u16> {
    ZIGZAG_LOC.get(index).copied()
}
...
mod tests {
    ...
    #[test]
    fn test_zigzag_bounds() {
        assert_eq!(ZIGZAG_LOC.len(), 1024);
        assert_eq!(get_zigzag_loc(0), 0);
        assert_eq!(get_zigzag_loc(1023), 1023);
    }

    #[test]
    fn test_zigzag_checked() {
        assert_eq!(get_zigzag_loc_checked(0), Some(0));
        assert_eq!(get_zigzag_loc_checked(1023), Some(1023));
        assert_eq!(get_zigzag_loc_checked(1024), None);
    }

    #[test]
    fn test_first_few_values() {
        // Test the first 16 values from the original C array
        let expected = [0, 16, 512, 528, 8, 24, 520, 536, 256, 272, 768, 784, 264, 280, 776, 792];
        for (i, &expected_val) in expected.iter().enumerate() {
            assert_eq!(get_zigzag_loc(i), expected_val);
        }
    }
}
