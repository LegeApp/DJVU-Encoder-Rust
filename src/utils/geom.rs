...
impl Rect {
    ...
    pub fn translate(&self, dx: i32, dy: i32) -> Rect {
        if self.is_empty() {
            ...
            Rect::new(self.x.saturating_add(dx), self.y.saturating_add(dy), self.width, self.height)
        }
    }
...
struct Ratio { p: i64, q: i64 }

impl Ratio {
    fn new(p: i32, q: i32) -> Result<Self> {
        if q == 0 {
            return Err(DjvuError::InvalidArg("Ratio denominator cannot be zero.".to_string()));
        }
        ...
        Ok(Ratio { p: p_64 / common, q: q_64 / common })
    }
}
...
impl RectMapper {
    ...
    pub fn set_input(&mut self, rect: Rect) -> Result<()> {
        if rect.is_empty() {
            return Err(DjvuError::InvalidArg("Input rectangle cannot be empty.".to_string()));
        }
        self.from = rect;
    ...
    pub fn set_output(&mut self, rect: Rect) -> Result<()> {
        if rect.is_empty() {
            return Err(DjvuError::InvalidArg("Output rectangle cannot be empty.".to_string()));
        }
        self.to = rect;
        self.recalculate_ratios()?;
        Ok(())
    ...
    pub fn rotate(&mut self, count: i32) -> Result<()> {
        match count.rem_euclid(4) {
            1 => {
                let mirror_flag = if (self.code & Self::SWAPXY) != 0 { Self::MIRRORY } else { Self::MIRRORX };
                self.code ^= mirror_flag;
                self.code ^= Self::SWAPXY;
            },
            2 => self.code ^= Self::MIRRORX | Self::MIRRORY,
            3 => {
                let mirror_flag = if (self.code & Self::SWAPXY) != 0 { Self::MIRRORX } else { Self::MIRRORY };
                self.code ^= mirror_flag;
                self.code ^= Self::SWAPXY;
            },
            _ => {},
    ...
    fn recalculate_ratios(&mut self) -> Result<()> {
        let from_w = if (self.code & Self::SWAPXY) != 0 { self.from.height } else { self.from.width };
        let from_h = if (self.code & Self::SWAPXY) != 0 { self.from.width } else { self.from.height };
    ...
    pub fn map(&self, x: i32, y: i32) -> (i32, i32) {
        ...
        if (self.code & Self::SWAPXY) != 0 { mem::swap(&mut mx, &mut my); }
        
        let from_w = if (self.code & Self::SWAPXY) != 0 { self.from.height } else { self.from.width } as i64;
        let from_h = if (self.code & Self::SWAPXY) != 0 { self.from.width } else { self.from.height } as i64;
        let from_x = if (self.code & Self::SWAPXY) != 0 { self.from.y } else { self.from.x } as i64;
        let from_y = if (self.code & Self::SWAPXY) != 0 { self.from.x } else { self.from.y } as i64;

        if (self.code & Self::MIRRORX) != 0 { mx = from_x + (from_x + from_w) - mx; }
        if (self.code & Self::MIRRORY) != 0 { my = from_y + (from_y + from_h) - my; }

        let out_x = self.to.x as i64 + ((mx - from_x) * self.ratio_w.p + self.ratio_w.q / 2) / self.ratio_w.q;
        let out_y = self.to.y as i64 + ((my - from_y) * self.ratio_h.p + self.ratio_h.q / 2) / self.ratio_h.q;
    ...
    pub fn map_rect(&self, rect: Rect) -> Rect {
        ...
        Rect::new(x1.min(x2), y1.min(y2), (x1 - x2).abs() as u32, (y1 - y2).abs() as u32)
    }
}